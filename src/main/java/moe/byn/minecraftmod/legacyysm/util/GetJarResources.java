...
public final class GetJarResources {
    ...
    public static void copyYesSteveModelFile(String filePath, Path destPath, String fileName) {
        ...
        if (url == null) {
            YesSteveModel.LOGGER.warn("Resource not found: {}", filePath);
            return;
        }
        YesSteveModel.LOGGER.info("Copying resource: {} to {}", filePath, destPath.resolve(fileName));
        try {
            FileUtils.copyURLToFile(url, destPath.resolve(fileName).toFile());
        } catch (IOException e) {
            YesSteveModel.LOGGER.error("Failed to copy resource: {}", filePath, e);
        }
    }
}
